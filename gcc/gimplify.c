...
gimplify_call_expr (tree *expr_p, gimple_seq *pre_p, bool want_value)
{
  ...
  if (nargs > 0)
	...
	= !!FUNCTION_PUSH_ARGS_REVERSED (fnptrtype ? TREE_TYPE (fnptrtype)
						   : NULL_TREE);
...
gimplify_function_tree (tree fndecl)
{
  ...
  if (flag_instrument_function_entry_exit
	...
	{
	  x = builtin_decl_implicit (BUILT_IN_PROFILE_FUNC_EXIT_SIMPLE);
      ...
      else
	{
      ...
      if (flag_instrument_function_entry_exit_simple)
	{
	  x = builtin_decl_implicit (BUILT_IN_PROFILE_FUNC_ENTER_SIMPLE);
	  call = gimple_build_call (x, 0);
	}
      else
	{
