...
(define_insn "*call"
	[(call (mem:QI (match_operand:HI 0 "general_operand" "g"))
	       (match_operand:HI  1 "general_operand" "g"))]
	"!SIBLING_CALL_P (insn)"
{
  if (ia16_far_function_rtx_p (operands[0]))
    {
      if (MEM_P (operands[0]))
...
(define_insn "*sibcall"
        ...
        "SIBLING_CALL_P (insn)"
{
  int far_caller = ia16_in_far_function_p ();
  int far_callee = ia16_far_function_rtx_p (operands[0]);
  if (! far_caller && far_callee)
    {
      if (MEM_P (operands[0]))
	return ("pushw\t%%cs\;jmp\t*%0");
      else if (CONSTANT_P (operands[0]))
	return ("pushw\t%%cs\;jmp\t%c0");
      else
	return ("pushw\t%%cs\;jmp\t*%0");
    }
  else
...
(define_insn "*call_value"
	[(set (match_operand 0 "register_operand" "=r")
	      (call (mem:QI (match_operand:HI 1 "general_operand" "g"))
	            (match_operand:HI  2 "general_operand" "g")))]
	"!SIBLING_CALL_P (insn)"
{
  if (ia16_far_function_rtx_p (operands[1]))
    {
      if (MEM_P (operands[1]))
...
{
  int far_caller = ia16_in_far_function_p ();
  int far_callee = ia16_far_function_rtx_p (operands[1]);
  if (! far_caller && far_callee)
    {
      if (MEM_P (operands[1]))
	return ("pushw\t%%cs\;jmp\t*%1");
      else if (CONSTANT_P (operands[1]))
	return ("pushw\t%%cs\;jmp\t%c1");
      else
	return ("pushw\t%%cs\;jmp\t*%1");
    }
  else
