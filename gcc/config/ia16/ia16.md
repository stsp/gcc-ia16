...
; Possible 64-bit return types for the `regparmcall' calling convention.
(define_mode_iterator EQ64 [DI DF])
...
;
; TODO: should the split occur even when not using -mregparmcall?  -- tkchia
...
; Implementing 64-bit return values in the `regparmcall' calling convention.
(define_insn "*call_value_64"
  [(set (parallel:EQ64 [(expr_list (match_operand:SI 0 "register_operand" "=r")
				   (const_int 0))
			(expr_list (match_operand:HI 1 "register_operand" "=r")
				   (const_int 4))
			(expr_list (match_operand:HI 2 "register_operand" "=r")
				   (const_int 6))])
	(call (mem:QI (match_operand:FNP 3 "general_operand" "<p>"))
	      (match_operand:HI 4 "general_operand" "g")))]
  "!SIBLING_CALL_P (insn)"
{
  return ia16_get_call_expansion (operands[3], <MODE>mode, 3);
})

(define_insn "*call_value_pop_64"
  [(parallel
    [(set (parallel:EQ64
	   [(expr_list (match_operand:SI 0 "register_operand" "=r")
		       (const_int 0))
	    (expr_list (match_operand:HI 1 "register_operand" "=r")
		       (const_int 4))
	    (expr_list (match_operand:HI 2 "register_operand" "=r")
	  ...
	  (call (mem:QI (match_operand:FNP 3 "general_operand" "<p>"))
		(match_operand:HI 4 "general_operand" "g")))
     (set (reg:HI SP_REG) (plus:HI (reg:HI SP_REG) (match_operand:HI 5)))]
  )]
  "!SIBLING_CALL_P (insn)"
{
  return ia16_get_call_expansion (operands[3], <MODE>mode, 3);
...
(define_insn "*sibcall_value_64"
  ...
  "SIBLING_CALL_P (insn)"
{
  return ia16_get_sibcall_expansion (operands[3], <MODE>mode, 3);
})

(define_insn "*sibcall_value_pop_64"
  [(parallel
    [(set (parallel:EQ64
	   [(expr_list (match_operand:SI 0 "register_operand" "=r")
		       (const_int 0))
