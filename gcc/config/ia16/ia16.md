...
;
; And, only activate this template if the register renaming pass (rnreg) is
; turned off.  rnreg does some really weird things with pseudo-registers,
; and I have not quite figured out how to make the "movmemhi" templates play
; well with it.
(define_expand "movmemhi"
  ...
  "! fixed_regs[DS_REG] && ! flag_rename_registers"
{
  rtx ovr, off, count;
  ...
  if (optimize == 0 || ! CONST_INT_P (operands[2]))
    {
      count = copy_to_mode_reg (HImode, operands[2]);
      operands[2] = count;
    }
  else
    count = gen_reg_rtx (HImode);
  operands[8] = count;
})
...
(define_insn "*movmemhi_far_from_far_const_count"
  [(parallel
    [(set (mem:BLK (plus:HI (match_operand:HI 0 "general_operand" "+D")
     ...
     (clobber (match_dup 0))
     (clobber (match_dup 2))
     (clobber (match_operand:HI 6 "" "=&c"))
     (clobber (reg:CC CC_REG))]
  )]
  "optimize > 0"
{
  if (INTVAL (operands[4]) % 2 == 0)
    return "movw\t%W4,\t%6\;cld\;rep; movsw";
  else
    return "movw\t%W4,\t%6\;cld\;rep; movsw\;movsb";
})
  ...
  [(parallel
    [(set (mem:BLK (plus:HI (match_operand:HI 0 "general_operand" "+D")
	  ...
	  (mem:BLK (plus:HI (match_operand:HI 2 "general_operand" "+S")
			    (unspec:HI [(match_operand:HI 3 "general_operand"
					 "Rds")] UNSPEC_SEG_OVERRIDE))))
     (use (match_operand:HI 4 "nonmemory_operand" "c"))
     (use (match_operand:HI 5 "const_int_operand" "n"))
     (clobber (match_dup 0))
     (clobber (match_dup 2))
     (clobber (match_operand:HI 6 "" "=4"))
     (clobber (reg:CC CC_REG))]
  )]
  ""
{
...
(define_insn_and_split "*movmemhi_far_from_near"
  [(parallel
    [(set (mem:BLK (plus:HI (match_operand:HI 0 "general_operand")
     ...
     (use (match_operand:HI 3 "nonmemory_operand"))
     (use (match_operand:HI 4 "const_int_operand"))
     (clobber (match_dup 0))
     (clobber (match_dup 2))
     (clobber (match_operand:HI 5 ""))
     (clobber (reg:CC CC_REG))]
  )]
  ...
  [(parallel
    [(set (mem:BLK (plus:HI (match_dup 0)
	  ...
	  (mem:BLK (plus:HI (match_dup 2)
			    (unspec:HI [(match_dup 6)] UNSPEC_SEG_OVERRIDE))))
     (use (match_dup 3))
     (use (match_dup 4))
     (clobber (match_dup 0))
     (clobber (match_dup 2))
...
{
  operands[6] = copy_to_reg (gen_rtx_REG (HImode, SS_REG));
})
...
(define_insn_and_split "*movmemhi_near_from_near"
  ...
  [(parallel
    [(set (mem:BLK (plus:HI (match_dup 0)
			    (unspec:HI [(match_dup 5)] UNSPEC_SEG_OVERRIDE)))
	  (mem:BLK (plus:HI (match_dup 1)
			    (unspec:HI [(match_dup 6)] UNSPEC_SEG_OVERRIDE))))
     (use (match_dup 2))
     (use (match_dup 3))
...
{
  operands[5] = copy_to_reg (gen_rtx_REG (HImode, SS_REG));
  operands[6] = copy_to_reg (gen_rtx_REG (HImode, SS_REG));
})
