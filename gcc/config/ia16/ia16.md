...
; Collapse the multiword right-shift-by-1 instructions into one pattern.
(define_code_iterator any_shiftrt1 [ashiftrt lshiftrt])
(define_code_attr optab_shiftrt1 [(ashiftrt "ashr") (lshiftrt "lshr")])
...
; generalize the RTL.
(define_insn "_<optab_shiftrt1><mode>_const1_cc_for_carry"
  [(set (reg:CCC CC_REG)
	(compare:CCC
		...
		(match_dup 1)))
   (set (match_operand:MO 0 "nonimmediate_operand" "=<r>m")
	(any_shiftrt1:MO (match_dup 1) (const_int 1)))]
  ""
  "<mnemonic><s>\t%0"
)
...
(define_expand "<optab_shiftrt1><mode>3"
  [(parallel
  [(set (match_operand:GT16I 0 "nonimmediate_operand")
	(any_shiftrt1:GT16I (match_operand:GT16I 1 "general_operand")
			    (match_operand:GT16I 2 "general_operand")))
   (clobber (reg:CC CC_REG))]
  )]
  ""
{
  ...
  if (shift != 1)
    {
      if (CODE_FOR_<optab_shiftrt1><mode>3 == CODE_FOR_ashr<mode>3
	  && shift == GET_MODE_BITSIZE (<MODE>mode) - 1)
	{
	  /* We _must_ tell GCC how to extract operand[1]'s sign bit and
	     spread it around operand[0], or else GCC will go into infinite
	     recursion and crash...  */
	  signed int i = GET_MODE_SIZE (<MODE>mode) - UNITS_PER_WORD;
	  rtx w2 = simplify_gen_subreg (HImode, operands[1], <MODE>mode, i);
  ...
  if ((MEM_P (operands[0]) && ! MEM_P (operands[1]))
	...
	{
	  tmp0 = gen_reg_rtx (<MODE>mode);
	  emit_insn (gen_<optab_shiftrt1><mode>3 (tmp0, tmp1, const1_rtx));
	  tmp1 = tmp0;
	}
      emit_move_insn (operands[0], tmp1);
      DONE;
...
(define_insn_and_split "*<optab_shiftrt1><mode>_const1"
  [(set (match_operand:GT16I 0 "nonimmediate_operand" "=r,m")
	(any_shiftrt1:GT16I (match_operand:GT16I 1 "nonimmediate_operand"
						 "0,0")
			    (const_int 1)))
   (clobber (reg:CC CC_REG))]
  ""
  ...
  [(const_int 0)]
{
  ...
  op0 = simplify_gen_subreg (HImode, operands[0], <MODE>mode, i);
  op1 = simplify_gen_subreg (HImode, operands[1], <MODE>mode, i);
  emit_insn (gen__<optab_shiftrt1>hi_const1_cc_for_carry (op0, op1));
