...
(define_insn "*cmp<mode>_cccz_c"
  [(set (reg:CCCZ_C CC_REG)
	(compare:CCCZ_C (match_operand:MO 0 "nonimmediate_operand" "<r>m")
			(match_operand:MO 1 "immediate_operand" "i")))]
  ""
  "cmp<s>\t%1+1,\t%0"
)

; Comparisons against zero. They don't need to set the overflow flag.
(define_insn "*cmp<MO:mode>_const0_<SET_SZ:mode>"
  [(set (reg:SET_SZ CC_REG)
	(compare:SET_SZ (match_operand:MO 0 "nonimmediate_operand" "<r>,m")
			(const_int 0)))]
  ""
  "@
   and<s>\t%0,\t%0
...
(define_insn "*cmp<MO:mode>_<SET_SOCZ:mode>"
  [(set (reg:SET_SOCZ CC_REG)
	(compare:SET_SOCZ (match_operand:MO 0 "nonimmediate_operand" "<r>,m")
			  (match_operand:MO 1 "general_operand" "<g>,<r>i")))]
  "!MEM_P (operands[0]) || !MEM_P (operands[1])"
  "@
   cmp<s>\t%1,\t%0
   cmp<s>\t%1,\t%0"
