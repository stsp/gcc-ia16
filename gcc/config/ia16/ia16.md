  ...
  "! fixed_regs[DS_REG]"
{
  ...
  if (optimize == 0 || ! CONST_INT_P (operands[2]))
    count = operands[2] = copy_to_mode_reg (HImode, operands[2]);
  else
...
(define_insn "*movmemhi_far_from_far_const_count"
  [(parallel
    [(set (mem:BLK (plus:HI (match_operand:HI 0 "" "+D")
			    (unspec:HI [(match_operand:HI 1 "" "e")]
				       UNSPEC_SEG_OVERRIDE)))
	  (mem:BLK (plus:HI (match_operand:HI 2 "" "+S")
			    (unspec:HI [(match_operand:HI 3 "" "Rds")]
				       UNSPEC_SEG_OVERRIDE))))
...
(define_insn_and_split "*movmemhi_far_from_near"
  [(parallel
    [(set (mem:BLK (plus:HI (match_operand:HI 0 "")
			    (unspec:HI [(match_operand:HI 1 "")]
				       UNSPEC_SEG_OVERRIDE)))
	  (mem:BLK (match_operand:HI 2 "")))
...
(define_insn_and_split "*movmemhi_near_from_far"
  [(parallel
    [(set (mem:BLK (match_operand:HI 0 ""))
	  (mem:BLK (plus:HI (match_operand:HI 1 "")
			    (unspec:HI [(match_operand:HI 2 "")]
				       UNSPEC_SEG_OVERRIDE))))
     (use (match_operand:HI 3 ""))
     (use (match_operand:HI 4 "const_int_operand"))
     (clobber (match_dup 0))
