  ...
  if (! ia16_in_ss_data_function_p ())
    {
      ...
      rtx src = operands[1];
      while (SUBREG_P (src)
	     && (GET_MODE (src) == HImode || GET_MODE (src) == PHImode))
	src = SUBREG_REG (src);
...
;    "	In file included from vshuf-v8qi.c:23:0:
...
; As above, but with segment overrides.
(define_insn "*xlatqi2_subreg_far"
  [(set (subreg:QI (match_operand:HI 0 "register_operand" "=a") 0)
	(mem:QI
	 (plus:HI
	  (plus:HI
	   (zero_extend:HI (match_operand:QI 1 "single_register_operand" "0"))
	   (match_operand:HI 2 "single_nonmemory_operand" "b"))
	  (unspec:HI [(match_operand:SEG 3 "register_operand" "Q")]
		     UNSPEC_SEG_OVERRIDE))))]
  ""
  "xlat\t%3:(%2)"
)

(define_insn "*xlatqi2_strict_low_part_far"
  [(set (strict_low_part
	 (subreg:QI (match_operand:HI 0 "register_operand" "+a") 0))
	(mem:QI
	 (plus:HI
	  (plus:HI
	   ...
	   (match_operand:HI 1 "single_nonmemory_operand" "b"))
	  (unspec:HI [(match_operand:SEG 2 "register_operand" "Q")]
		     UNSPEC_SEG_OVERRIDE))))]
  ""
  "xlat\t%2:(%1)"
)

(define_insn "*xlatqi2_far"
...
{
  bool equal = rtx_equal_p (operands[0], operands[1]);
  if (! equal && memory_operand (operands[1], <MODE>mode))
    operands[1] = force_reg (<MODE>mode, operands[1]);
  else if (! equal && memory_operand (operands[0], <MODE>mode))
    {
      rtx tmp = gen_reg_rtx (<MODE>mode);
      emit_insn (gen_sub<mode>3 (tmp, operands[1], operands[2]));
      emit_move_insn (operands[0], tmp);
      DONE;
    }
})
...
; Insn patterns used in multiword left shift by 1.
...
(define_insn "*ashr<mode>_const1_cconly_for_carry"
  [(set (reg:CCC CC_REG)
	(compare:CCC
		(and:MO (match_operand:MO 1 "nonimmediate_operand" "0")
			(const_int -2))
...
(define_insn_and_split "*ashl<mode>_const1"
  [(set (match_operand:GT16I 0 "nonimmediate_operand" "=r,m")
...
(define_insn_and_split "*<optab_shiftrt_mw><mode>_const1"
  [(set (match_operand:GT16I 0 "nonimmediate_operand" "=r,m")
...
(define_insn "*lshrhi_const15_addhi3"
  ...
  ""
  "@
   sar{w\t%0|\t%0,\t1}\;rcl{w\t%0|\t%0,\t1}\;adc{w\t$0,\t%0|\t%0,\t0}
   cmp{b\t$0x80,\t%H2|\t%H2,\t0x80}\;cmc\;adc{w\t$0,\t%0|\t%0,\t0}
   cmp{w\t$0x8000,\t%2|\t%2,\t0x8000}\;cmc\;adc{w\t$0,\t%0|\t%0,\t0}"
)
...
(define_insn "*lshlsi_const1_twoparts"
  [(set (match_operand:HI 0 "register_operand" "=r")
   ...
   (clobber (reg:CC CC_REG))]
  ""
  "xor{w}\t%2,\t%2\;shl{w\t%0|\t%0,\t1}\;rcl{w\t%2|\t%2,\t1}"
)
...
; The splitting pattern is to stop GCC from crashing when building Newlib
; with -mregparmcall:
;
;    "	.../stdlib/dtoa.c: In function ‘_dtoa_r’:
;	.../stdlib/dtoa.c:872:1: error: unable to find a register to spill
;	 }
;	 ^
;	.../stdlib/dtoa.c:872:1: error: this is the insn:
;	(insn 7514 7607 7610 31 (set (reg:SI 3423 [orig:460 i ] [460])
;		(sign_extend:SI (subreg/j:HI (reg:SI 3423 [orig:460 i ] [460])
;	 0))) .../stdlib/dtoa.c:360 434 {*extendhisi2}
;	     (nil))  "
;
; (Apparently GCC had some sort of problem unifying the views of %dx:%ax as
; separate HImode registers with the view of %dx:%ax as a single SImode
; thing, when trying to pass the sign-extended result to a subroutine...)
;	-- tkchia
(define_insn_and_split "*extendhisi2"
	...
	"cw{t}d"
	""
	[(set (subreg:HI (match_dup 0) 0) (match_dup 1))
	 (set (subreg:HI (match_dup 0) 2)
			 (ashiftrt:HI (match_dup 1) (const_int 15)))]

)

(define_expand "extendqihi2"
	[(set (match_dup 2) (match_operand:QI 1 "register_operand"))
	 (set (match_operand:HI 0 "register_operand")
...
(define_insn "*extendqihi2"
	[(set (match_operand:HI 0 "register_operand" "=a")
	      (sign_extend:HI (match_operand:QI 1 "register_operand" "Ral")))]
	""
	"cb{t}w"
)
...
(define_insn "*seqqi_<USE_Z:mode>"
	...
	""
	"@
	lahf\;shl{b\t%1|\t%1,\t1}\;cw{t}d
	lahf\;shl{b\t%0|\t%0,\t1}\;mov{b\t%0,\t%1|\t%1,\t%0}\;cb{t}w"
)
...
(define_insn "*parityhi2"
  ...
  ""
  "@
  lahf\;and{b\t$0x4,\t%H0|\t%H0,\t0x4}\;add{b\t$0xfc,\t%H0|\t%H0,\t0xfc}\;sbb{w}\t%0,\t%0\;inc{w}\t%0
  lahf\;and{b\t$0x4,\t%1|\t%1,\t0x4}\;add{b\t$0xfc,\t%1|\t%1,\t0xfc}\;sbb{w}\t%0,\t%0\;inc{w}\t%0"
)
...
(define_insn "*parityqi2"
  ...
  ""
  "@
  lahf\;and{b\t$0x4,\t%0|\t%0,\t0x4}\;add{b\t$0xfc,\t%0|\t%0,\t0xfc}\;sbb{b}\t%0,\t%0\;inc{b}\t%0
  lahf\;and{b\t$0x4,\t%1|\t%0,\t0x4}\;add{b\t$0xfc,\t%1|\t%0,\t0xfc}\;sbb{b}\t%0,\t%0\;inc{b}\t%0"
)

; Activate this template only if %ds is available for allocation.  If %ds is
; configured to be a fixed-purpose register (`-ffixed-ds'), fall back on
; GCC's default treatment of memory copyings, whatever that may be.
;
; Also fall back on the default (i.e. memcpy (...)) if this is a move
; between two addresses in generic (near) space.
(define_expand "movmemhi"
  [(parallel
    [(set (match_operand:BLK 0 "general_operand")
     ...
     (set (match_dup 6) (plus:HI (match_dup 9) (match_dup 2)))
     (set (match_dup 7) (plus:HI (match_dup 10) (match_dup 2)))
     (set (match_dup 8) (const_int 0))
     (clobber (reg:CC CC_REG))]
  )]
  "! fixed_regs[DS_REG]"
{
  rtx ovr0, off0, ovr1, off1;

  ia16_split_seg_override_and_offset (XEXP (operands[0], 0), &ovr0, &off0);
  ia16_split_seg_override_and_offset (XEXP (operands[1], 0), &ovr1, &off1);
  ...
  if (! off0)
    off0 = const0_rtx;
  /* We need to at least massage the offset portions of operands[0] and
     operands[1] into expressions that satisfy "general_operand", or else
     GCC will have a hard time matching the insns below later on.  */
  off0 = copy_addr_to_reg (off0);
  if (! ovr0)
    {
      rtx seg;
      if ((ADDR_SPACE_GENERIC_P (MEM_ADDR_SPACE (operands[0]))
	   || MEM_READONLY_P (operands[0]))  /* FIXME: see ia16-no-ss-data.c */
	  && ! ia16_in_ss_data_function_p ())
	seg = ia16_data_seg_rtx ();
      else
	seg = gen_rtx_REG (SEGmode, SS_REG);
      ovr0 = ia16_seg_override_term (copy_to_reg (seg));
    }
  operands[0] = gen_rtx_MEM (BLKmode, gen_rtx_PLUS (HImode, off0, ovr0));
  set_mem_addr_space (operands[0], ADDR_SPACE_FAR);
  operands[9] = off0;

  if (! off1)
    off1 = const0_rtx;
  off1 = copy_addr_to_reg (off1);
  if (! ovr1)
    {
      rtx seg;
      if ((ADDR_SPACE_GENERIC_P (MEM_ADDR_SPACE (operands[1]))
	   || MEM_READONLY_P (operands[1]))  /* FIXME: see ia16-no-ss-data.c */
	  && ! ia16_in_ss_data_function_p ())
	seg = ia16_data_seg_rtx ();
      else
	seg = gen_rtx_REG (SEGmode, SS_REG);
      ovr1 = ia16_seg_override_term (copy_to_reg (seg));
    }
  operands[1] = gen_rtx_MEM (BLKmode, gen_rtx_PLUS (HImode, off1, ovr1));
  set_mem_addr_space (operands[1], ADDR_SPACE_FAR);
  operands[10] = off1;
})

(define_insn "*movmemhi_far_from_far_const_count<pmrm>"
  [(parallel
    [(set (mem:BLK (plus:HI (match_operand:HI 0 "register_operand" "D")
			    (unspec:HI [(match_operand:SEG 1 "register_operand"
					 "e")] UNSPEC_SEG_OVERRIDE)))
	  (mem:BLK (plus:HI (match_operand:HI 2 "register_operand" "S")
			    (unspec:HI [(match_operand:SEG 3 "register_operand"
					 "Rds")] UNSPEC_SEG_OVERRIDE))))
     (use (match_operand:HI 4 "const_int_operand" "n"))
     (use (match_operand:HI 5 "const_int_operand" "n"))
     (set (match_operand:HI 6 "register_operand" "=&0")
	  (match_operand:HI 9 ""))
     (set (match_operand:HI 7 "register_operand" "=&2")
	  (match_operand:HI 10 ""))
     (set (match_operand:HI 8 "register_operand" "=&c")
	  (match_operand:HI 11 ""))
     (clobber (reg:CC CC_REG))]
  )]
  "optimize > 0"
{
  if (INTVAL (operands[4]) % 2 == 0)
    return "mov{w\t%W4,\t%8|\t%8,\t%W4}\;cld\;rep; movsw";
  else
    return "mov{w\t%W4,\t%8|\t%8,\t%W4}\;cld\;rep; movsw\;movsb";
})

(define_insn "*movmemhi_far_from_far_nonconst_count<pmrm>"
  [(parallel
    [(set (mem:BLK (plus:HI (match_operand:HI 0 "register_operand" "D")
			    (unspec:HI [(match_operand:SEG 1 "register_operand"
					 "e")] UNSPEC_SEG_OVERRIDE)))
	  (mem:BLK (plus:HI (match_operand:HI 2 "register_operand" "S")
			    (unspec:HI [(match_operand:SEG 3 "register_operand"
					 "Rds")] UNSPEC_SEG_OVERRIDE))))
     (use (match_operand:HI 4 "general_operand" "c"))
     (use (match_operand:HI 5 "const_int_operand" "n"))
     (set (match_operand:HI 6 "register_operand" "=&0")
	  (match_operand:HI 9 ""))
     (set (match_operand:HI 7 "register_operand" "=&2")
	  (match_operand:HI 10 ""))
     (set (match_operand:HI 8 "register_operand" "=&4")
	  (match_operand:HI 11 ""))
     (clobber (reg:CC CC_REG))]
  )]
  ""
{
  if (optimize == 0 || optimize_size)
    return "cld\;rep; movsb";
  else
    return "cld\;shr{w\t%4|\t%4,\t1}\;rep; movsw\;rcl{w\t%4|\t%4,\t1}\;rep; movsb";
})

; Also handle memset (...)-like memory block set operations, for far blocks.
(define_expand "setmemhi"
  [(parallel
    [(set (match_operand:BLK 0 "general_operand")
	  (unspec:BLK [(match_operand:QI 2 "nonmemory_operand")]
		      UNSPEC_DUP_SETMEM))
     (use (match_operand:HI 1 "nonmemory_operand"))
     (use (match_operand:HI 3 "const_int_operand"))
     (set (match_dup 6) (plus:HI (match_dup 8) (match_dup 1)))
     (set (match_dup 7) (const_int 0))
     (clobber (reg:CC CC_REG))]
  )]
  ""
{
  ...
  ia16_split_seg_override_and_offset (XEXP (operands[0], 0), &ovr, &off);

  if (! ovr && ADDR_SPACE_GENERIC_P (MEM_ADDR_SPACE (operands[0])))
    FAIL;
  ...
  if (! ovr)
	...
	seg = gen_rtx_REG (SEGmode, SS_REG);
      ovr = ia16_seg_override_term (copy_to_reg (seg));
    }
  operands[0] = gen_rtx_MEM (BLKmode, gen_rtx_PLUS (HImode, off, ovr));
  set_mem_addr_space (operands[0], ADDR_SPACE_FAR);

  val = operands[2];
  if (CONST_INT_P (val))
    {
      HOST_WIDE_INT v = INTVAL (val) & 0xff;
      v |= v << 8;
      val = gen_int_mode (v, HImode);
    }
  else
    {
      val = convert_modes (HImode, QImode, val, true);
      tmp = expand_simple_binop (HImode, ASHIFT, val, GEN_INT (8), NULL,
				 true, OPTAB_DIRECT);
      val = expand_simple_binop (HImode, IOR, val, tmp, val, true,
				 OPTAB_DIRECT);
    }
  operands[2] = val;
...
(define_insn "*setmemhi_far_from_hi_const_count<pmrm>"
  [(parallel
    [(set (mem:BLK (plus:HI (match_operand:HI 0 "register_operand" "D")
			    (unspec:HI [(match_operand:SEG 1 "register_operand"
					 "e")] UNSPEC_SEG_OVERRIDE)))
	  (unspec:BLK [(match_operand:HI 2 "general_operand" "a")]
		      UNSPEC_DUP_SETMEM))
     (use (match_operand:HI 3 "const_int_operand" "n"))
     (use (match_operand:HI 4 "const_int_operand" "n"))
     (set (match_operand:HI 5 "register_operand" "=&0")
	  (match_operand:HI 7 ""))
     (set (match_operand:HI 6 "register_operand" "=&c")
	  (match_operand:HI 8 ""))
     (clobber (reg:CC CC_REG))]
  )]
  "optimize > 0"
{
  if (INTVAL (operands[3]) % 2 == 0)
    return "mov{w\t%W3,\t%6|\t%6,\t%W3}\;cld\;rep; stosw";
  else
    return "mov{w\t%W3,\t%6|\t%6,\t%W3}\;cld\;rep; stosw\;stosb";
})

(define_insn "*setmemhi_far_from_hi_nonconst_count<pmrm>"
  [(parallel
    [(set (mem:BLK (plus:HI (match_operand:HI 0 "register_operand" "D")
			    (unspec:HI [(match_operand:SEG 1 "register_operand"
					 "e")] UNSPEC_SEG_OVERRIDE)))
	  (unspec:BLK [(match_operand:HI 2 "general_operand" "a")]
		      UNSPEC_DUP_SETMEM))
     (use (match_operand:HI 3 "general_operand" "c"))
     (use (match_operand:HI 4 "const_int_operand" "n"))
     (set (match_operand:HI 5 "register_operand" "=&0")
	  (match_operand:HI 7 ""))
     (set (match_operand:HI 6 "register_operand" "=&3")
	  (match_operand:HI 8 ""))
     (clobber (reg:CC CC_REG))]
  )]
  ""
  "cld\;shr{w\t%3|\t%3,\t1}\;rep; stosw\;rcl{w\t%3|\t%3,\t1}\;rep; stosb"
)

; Various conditional branches.
(define_expand "cbranch<mode>4"
  [(set (reg:CC CC_REG)
	(compare:CC (match_operand:MO 1 "nonimmediate_operand")
		    (match_operand:MO 2 "general_operand")))
   (set (pc) (if_then_else
  ...
  ""
{
  if (MEM_P (operands[1]) && MEM_P (operands[2]))
    operands[1] = force_reg (<MODE>mode, operands[1]);
  ia16_gen_compare_reg (GET_CODE (operands[0]), operands[1], operands[2],
  		        true);
})
...
(define_expand "call"
  [(call (match_operand:QI 0 "")
   ...
   (use (match_operand:HI 2))]
  ""
{
  rtx addr = XEXP (operands[0], 0);
  if (! operands[2])
    operands[2] = const0_rtx;  /* FIXME? */
  else if (CONST_INT_P (operands[2]) && INTVAL (operands[2]))
    operands[0] = gen_rtx_MEM (PHImode, addr);
  ia16_expand_reset_ds_for_call (addr);
})
...
(define_expand "call_pop"
  [(parallel
    [(call (match_operand:QI 0 "")
	   (match_operand:HI 1 "general_operand"))
     (set (reg:HI SP_REG) (plus:HI (reg:HI SP_REG) (match_operand:HI 3)))]
  )]
  ""
{
  ia16_expand_reset_ds_for_call (XEXP (operands[0], 0));
})
...
(define_insn "*call"
	[(call (mem:QI (match_operand:FNP 0 "general_operand" "<p>"))
	       (match_operand:HI  1 "general_operand" "g"))]
	"!SIBLING_CALL_P (insn)
	 && ! ia16_use_far_thunked_call_p (operands[0], <MODE>mode)"
{
  return ia16_get_call_expansion (operands[0], <MODE>mode, 0);
})

(define_insn "*call_isr"
	[(call (mem:PHI (match_operand:FNP 0 "general_operand" "<p>"))
	       (match_operand:HI  1 "general_operand" "g"))]
	"!SIBLING_CALL_P (insn)
	 && ! ia16_use_far_thunked_call_p (operands[0], <MODE>mode)"
{
  return ia16_get_isr_call_expansion (operands[0], <MODE>mode, 0);
})

(define_insn "*call_far_thunked"
	[(call (mem:QI (match_operand:HI 0 "general_operand" "b,b"))
	       (match_operand:HI 1 "general_operand" "Ifa,x"))]
	"!SIBLING_CALL_P (insn)
	 && ia16_use_far_thunked_call_p (operands[0], HImode)"
{
  return ia16_get_far_thunked_call_expansion (0, false);
})
...
(define_insn "*call_pop_far_thunked"
	[(parallel
	  [(call (mem:QI (match_operand:HI 0 "general_operand" "b,b,b"))
	         (match_operand:HI 1 "general_operand" "Ifa,x,x"))
	   (set (reg:HI SP_REG) (plus:HI (reg:HI SP_REG)
					 (match_operand:HI 2 "general_operand"
							     "1,1,x")))]
	)]
	"!SIBLING_CALL_P (insn)
	 && ia16_use_far_thunked_call_p (operands[0], HImode)"
{
  return ia16_get_far_thunked_call_expansion (0, true);
})
...
(define_insn "*sibcall"
        ...
        "SIBLING_CALL_P (insn)"
{
  return ia16_get_sibcall_expansion (operands[0], <MODE>mode, 0);
})
...
(define_expand "call_value"
  [(set (match_operand 0 "")
	(call (match_operand:QI 1 "")
	      (match_operand:HI 2 "general_operand")))
   (use (match_operand:HI 3))]
  ""
{
  rtx addr = XEXP (operands[1], 0);
  if (! operands[3])
    operands[3] = const0_rtx;  /* FIXME? */
  else if (CONST_INT_P (operands[3]) && INTVAL (operands[3]))
    {
      addr_space_t as = MEM_ADDR_SPACE (operands[1]);
      operands[1] = gen_rtx_MEM (PHImode, addr);
    }
  ia16_expand_reset_ds_for_call (addr);
})
...
(define_expand "call_value_pop"
  ...
  ""
{
  ia16_expand_reset_ds_for_call (XEXP (operands[1], 0));
})
...
(define_insn "*call_value"
	[(set (match_operand 0 "register_operand" "=r")
	      (call (mem:QI (match_operand:FNP 1 "general_operand" "<p>"))
	            (match_operand:HI  2 "general_operand" "g")))]
	"!SIBLING_CALL_P (insn)
	 && ! ia16_use_far_thunked_call_p (operands[1], <MODE>mode)"
{
  return ia16_get_call_expansion (operands[1], <MODE>mode, 1);
})

(define_insn "*call_value_isr"
	[(set (match_operand 0 "register_operand" "=r")
	      (call (mem:PHI (match_operand:FNP 1 "general_operand" "<p>"))
	            (match_operand:HI  2 "general_operand" "g")))]
	"!SIBLING_CALL_P (insn)
	 && ! ia16_use_far_thunked_call_p (operands[1], <MODE>mode)"
{
  return ia16_get_isr_call_expansion (operands[1], <MODE>mode, 1);
})

(define_insn "*call_value_far_thunked"
	[(set (match_operand 0 "register_operand" "=r,r")
	      (call (mem:QI (match_operand:HI 1 "general_operand" "b,b"))
	            (match_operand:HI  2 "general_operand" "Ifa,x")))]
	"!SIBLING_CALL_P (insn)
	 && ia16_use_far_thunked_call_p (operands[1], HImode)"
{
  return ia16_get_far_thunked_call_expansion (1, false);
})
...
(define_insn "*call_value_pop_far_thunked"
	[(parallel
	  [(set (match_operand 0 "register_operand" "=r,r,r")
		(call (mem:QI (match_operand:HI 1 "general_operand" "b,b,b"))
		      (match_operand:HI  2 "general_operand" "Ifa,x,x")))
	   (set (reg:HI SP_REG) (plus:HI (reg:HI SP_REG)
	...
	"!SIBLING_CALL_P (insn)
	 && ia16_use_far_thunked_call_p (operands[1], HImode)"
{
  return ia16_get_far_thunked_call_expansion (1, true);
})
...
(define_insn "*sibcall_value"
        ...
        "SIBLING_CALL_P (insn)"
{
  return ia16_get_sibcall_expansion (operands[1], <MODE>mode, 1);
})
...
; Keep ia16_initial_frame_pointer_offset() in sync with this pattern.
; Keep ia16_initial_arg_pointer_offset() in sync with this pattern.
(define_expand "prologue"
  [(const_int 1)]
  ""
  "ia16_expand_prologue (); DONE;")
...
(define_expand "epilogue"
  [(const_int 0)]
  ""
  "ia16_expand_epilogue (false); DONE;")

(define_expand "sibcall_epilogue"
  [(const_int 0)]
  ""
  "ia16_expand_epilogue (true); DONE;")
...
(define_expand "simple_return"
  ...
  "reload_completed"
{
  int pops = crtl->args.pops_args;
  ia16_expand_reset_ds_for_return ();
  if (pops)
    {
      emit_jump_insn (gen__simple_return_with_pops (GEN_INT (pops)));
      DONE;
    }
})
  ...
  "reload_completed"
{
  if (! ia16_in_far_function_p ())
    return "ret";
  else if (! ia16_in_interrupt_function_p ())
    return "{lret|retf}";
  else
    return "iret";
})

(define_insn "_simple_return_with_pops"
  [(return)
...
;; Far addressing support.
(define_expand "seg_override<pmrm>"
  [(unspec:HI [(match_operand:SEG 0 "" "")] UNSPEC_SEG_OVERRIDE)]
  ""
  ""
)

(define_expand "seg16_reloc<pmrm>"
  [(unspec:SEG [(match_operand:HI 0 "" "")] UNSPEC_OZSEG16)]
  ""
  ""
)

(define_insn "*mov<mode>_seg16<pmrm>"
  [(set (match_operand:SEG 0 "register_operand" "=r")
	(unspec:SEG [(match_operand:HI 1 "immediate_operand" "")]
		    UNSPEC_OZSEG16))]
  ""
{
  if (TARGET_ABI_SEGELF)
    return "mov{w\t$%Z1@SEG,\t%0|\t%0,\t%Z1@SEG}";
  else
    return "mov{w\t%1@OZSEG16,\t%0|\t%0,\t%1@OZSEG16}";
})

(define_insn "*movhi_seg16_via_paradoxical"
  [(set (match_operand:HI 0 "register_operand" "=r")
	(subreg:HI (unspec:PHI [(match_operand:HI 1 "immediate_operand" "")]
			       UNSPEC_OZSEG16)
		   0))]
  ""
{
...
(define_expand "static_far_ptr"
  [(const:SI (unspec:SI [(match_operand:HI 0 "" "")] UNSPEC_STATIC_FAR_PTR))]
  ""
  ""
)

;; Protected mode support (experimental, continued).
(define_insn "trunchiphi2"
  [(set (match_operand:PHI 0 "nonimmediate_operand" "=T,rm,Q,!Q")
	(truncate:PHI (match_operand:HI 1 "nonimmediate_operand"
					  "rm,Ti,0,Q")))]
  ""
  "@
   mov{w\t%1,\t%0|\t%0,\t%1}
...
(define_insn "extendphihi2"
  [(set (match_operand:HI 0 "nonimmediate_operand" "=r,rm")
	(sign_extend:HI (match_operand:PHI 1 "general_operand" "rm,Ti")))]
  ""
{
...
(define_insn "zero_extendphihi2"
  [(set (match_operand:HI 0 "nonimmediate_operand" "=r,rm")
	(zero_extend:HI (match_operand:PHI 1 "general_operand" "rm,Ti")))]
  ""
{
  if (REG_P (operands[0]) && REG_P (operands[1])
      && REGNO(operands[0]) == REGNO (operands[1]))
...
(define_expand "_pushphi1_prologue"
  [(parallel
  [(set (match_dup 1) (match_operand:PHI 0 "general_operand"))
   (set (reg:HI SP_REG) (plus:HI (reg:HI SP_REG) (const_int -2)))]
  )]
  ""
{
  operands[1] = gen_frame_mem (PHImode,
			       plus_constant (Pmode, stack_pointer_rtx, -2));
})
