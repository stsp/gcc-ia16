  ...
  "! fixed_regs[DS_REG]"
{
  ...
  if (! ovr)
    operands[0] = gen_rtx_MEM (BLKmode, off);
  else
    {
  ...
  ia16_split_seg_override_and_offset (XEXP (operands[1], 0), &ovr, &off);
  if (! off)
    off = const0_rtx;
  off = copy_addr_to_reg (off);
  if (! ovr)
    operands[1] = gen_rtx_MEM (BLKmode, off);
  else
    {
      operands[1] = gen_rtx_MEM (BLKmode, gen_rtx_PLUS (HImode, off, ovr));
...
(define_insn_and_split "*movmemhi_near_from_near"
  [(parallel
    [(set (mem:BLK (match_operand:HI 0 "general_operand"))
	  (mem:BLK (match_operand:HI 1 "general_operand")))
     (use (match_operand:HI 2 "general_operand"))
     (use (match_operand:HI 3 "const_int_operand"))
     (set (match_operand:HI 4 "nonimmediate_operand") (match_operand:HI 7 ""))
     (set (match_operand:HI 5 "nonimmediate_operand") (match_operand:HI 8 ""))
     (set (match_operand:HI 6 "nonimmediate_operand") (match_operand:HI 9 ""))
     (clobber (reg:CC CC_REG))]
  )]
  ...
  [(parallel
    [(set (mem:BLK (plus:HI (match_dup 0)
			    (unspec:HI [(match_dup 10)] UNSPEC_SEG_OVERRIDE)))
	  (mem:BLK (plus:HI (match_dup 1)
			    (unspec:HI [(match_dup 11)] UNSPEC_SEG_OVERRIDE))))
     (use (match_dup 2))
     (use (match_dup 3))
...
{
  operands[10] = copy_to_reg (gen_rtx_REG (HImode, SS_REG));
  operands[11] = copy_to_reg (gen_rtx_REG (HImode, SS_REG));
})
